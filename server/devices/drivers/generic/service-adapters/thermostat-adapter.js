...
class GenericThermostatAdapter extends GenericServiceAdapter {
	_adaptState (state) {
	...
	_adaptSocketEmit (event, data, callback = () => { /* no-op */ }) {
		...
		console.log("[thermostat-adapter] event", event);
		switch (event) {
			case 'action':
				if (data.property === 'light_level') {
					adapted_event = 'thermostat';
...
GenericThermostatAdapter.generic_type = 'thermostat';
GenericThermostatAdapter.relay_type = 'thermostat';
GenericThermostatAdapter.settings_definitions = new Map([...GenericServiceAdapter.settings_definitions])
	.set('schedule', {
		type: 'list-of',
		label: 'Schedule',
...
module.exports = GenericThermostatAdapter;
