	...
	_adaptSocketEmit (event, data, callback = () => { /* no-op */ }) {
		...
		switch (event) {
			case 'action':
				...
				break;
			case 'settings':
				console.log("sending current mode: ",data.settings.current_mode);
				if ('current_mode' in data.settings) {
					adapted_event = 'global-alarm';
					adapted_data = {globalAlarmMode: this._adaptModeToDevice(data.settings.current_mode)};
				}
