...
class GenericAccessControlAdapter extends GenericServiceAdapter {
	_adaptState (state) {
	...
	_adaptSocketEmit (event, data, callback = () => { /* no-op */ }) {
		...
		console.log("[access-control-adapter] event", event);
		switch (event) {
			case 'action':
				if (data.property === 'light_level') {
					adapted_event = 'access-control';
...
GenericAccessControlAdapter.generic_type = 'access-control';
GenericAccessControlAdapter.relay_type = 'access-control';
GenericAccessControlAdapter.settings_definitions = new Map([...GenericServiceAdapter.settings_definitions])
	.set('schedule', {
		type: 'list-of',
		label: 'Schedule',
