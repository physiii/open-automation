...
class GenericGlobalAlarmAdapter extends GenericServiceAdapter {
	_adaptState (state) {
		return GenericServiceAdapter.prototype._adaptState.call(this, {
			...state,
			c: this._adaptLevelToRelay(state.globalAlarmLevel)
		});
	}
	...
	_adaptSocketEmit (event, data, callback = () => { /* no-op */ }) {
		...
		console.log("global-alarm-adapter _adaptSocketEmit",event);
		switch (event) {
			case 'action':
				if (data.property === 'globalAlarmLevel') {
					adapted_event = 'global-alarm';
					adapted_data = {globalAlarmLevel: this._adaptLevelToDevice(data.value)};
				}
				break;
			case 'settings':
				should_emit = false;
	...
	_adaptLevelToDevice (globalAlarmLevel) {
		// Convert 0-1 percentage scale to value range of the globalAlarmLevel property on the device.
		return Math.round(globalAlarmLevel * LEVEL_SCALE);
	}

	_adaptLevelToRelay (globalAlarmLevel) {
		// Convert globalAlarmLevel property to a percentage between 0 and 1.
		return Math.round((globalAlarmLevel / LEVEL_SCALE) * 100) / 100;
	}
	...
	_sendSchedules (new_schedules = []) {
		return new Promise((resolve, reject) => {
			this._events.emit(this._getPrefixedEvent('settings/get'), {}, (error, {settings}) => {
				...
				for (const {seconds_into_day, globalAlarmLevel, on, should_add, should_delete} of schedule_changes) {
					if (device_errored) {
						return;
					}

					const schedule_id = this._getScheduleId(seconds_into_day, globalAlarmLevel, on),
						payload = {
					...
					if (should_add) {
						...
						payload.state = {on, globalAlarmLevel};
	...
	_adaptSchedules (schedules, added_or_removed) {
		...
		const adapted_schedules = schedules.map((schedule) => {
			...
			if (utils.isEmpty(schedule.time) || utils.isEmpty(schedule.globalAlarmLevel) || !time.isValid()) {
			...
			return {
				...
				globalAlarmLevel: this._adaptLevelToDevice(schedule.globalAlarmLevel),
				on: true,
		...
		return schedules_to_filter.filter((filter_schedule) => {
			return !schedules_to_compare.find((compare_schedule) => {
				return compare_schedule.time === filter_schedule.time &&
					compare_schedule.globalAlarmLevel === filter_schedule.globalAlarmLevel &&
					compare_schedule.on === filter_schedule.on;
			});
		});
	}

	_getScheduleId (time, globalAlarmLevel, on) {
		return crypto.createHash('md5').update(String(time) + String(globalAlarmLevel) + String(on)).digest('hex').substring(0, 15);
	}
};

GenericGlobalAlarmAdapter.generic_type = 'global-alarm';
GenericGlobalAlarmAdapter.relay_type = 'global-alarm';
GenericGlobalAlarmAdapter.settings_definitions = new Map([...GenericServiceAdapter.settings_definitions])
	.set('schedule', {
		type: 'list-of',
		label: 'Schedule',
		...
		main_property: 'time',
		secondary_property: 'globalAlarmLevel',
		sort_by: 'time',
		validation: {is_required: false}
	});

module.exports = GenericGlobalAlarmAdapter;
